...
export default {
  ...
  methods: {
    ...
    getRouteBreadcrumb() {
      ...
      routes
        .filter(
          (item) =>
            ...
            path.includes(item.path.substring(0, path.indexOf(":")))
        )
        .forEach((route) => {
          const path = route.path.length === 0 ? "/home" : route.path;
          breadcrumb.push({
            name: this.$t(getI18nKey(path)),
            path,
          });
        });
      let pageTitle = this.page && this.page.title;
      if (this.customTitle || pageTitle) {
        breadcrumb[breadcrumb.length - 1] = {
          name: this.customTitle || pageTitle,
          path,
        };
      }
      return breadcrumb;
    ...
    updatePageHeight(
      ...
      console.log("updatePageHeight:", this.pageHeaderHeight);
    },
  },
};
</script>
